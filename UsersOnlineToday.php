 ...
 * This is what creates a the admin options under miscellaneous configuration settings.
 *
 * Called by: integrate_general_mod_settings
 */
function UsersOnlineToday_settings(&$config_vars)
{
	global $txt;
	...
	$config_vars[] = '' ;
	$config_vars[] = $txt['uot_modname'];
	$config_vars[] = array('select', 'uot_setting_sortby', array('username' => $txt['uot_setting_username'], 'login_time' => $txt['uot_setting_login_time']));
	$config_vars[] = array('select', 'uot_setting_sortorder', array('ascending' => $txt['uot_setting_ascending'], 'descending' => $txt['uot_setting_descending']), 'postinput' => $txt['uot_setting_sortorder_post_txt']);
	$config_vars[] = array('select', 'uot_setting_period', array('current_day' => $txt['uot_setting_current_day'], 'last_24_hours' => $txt['uot_setting_last_24_hours'], 'last_7_days' => $txt['uot_setting_last_7_days']));
	$config_vars[] = array('select', 'uot_setting_canview', array('admin' => $txt['uot_setting_admin'], 'registered' => $txt['uot_setting_registered'], 'everyone' => $txt['uot_setting_everyone']));
}

/**
 * Retrieve a list and several other statistics of the users online during a specific period.
 *
 * Called by: integrate_mark_read_button
 */
function UsersOnlineToday_addStats()
{
	global $smcFunc, $context, $scripturl, $user_info, $modSettings, $txt;

	loadLanguage('UsersOnlineToday');
	loadTemplate('UsersOnlineToday');

	// Add the item to the info center.
	$context['info_center'][] = [
        'tpl' => 'UsersOnlineToday',
        'txt' => 'uot_users_online_' . $modSettings['uot_setting_period'],
    ];

	// Retrieve the data for the mod
	...
	$context += $usersOnlineTodayStats;
}

/**
 * Mention who made this thing
 * 
 * Called by: integrate_credits
 */
function UsersOnlineToday_credits()
{
	global $context;
	$context['copyrights']['mods'][] = 'Users Online Today for SMF2.1 by @rjen &copy; 2022';
}
?>
